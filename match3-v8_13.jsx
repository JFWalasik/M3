...
// =============================================================================
// MATCH-3 GAME v8.13 - CANVAS EDITION
// Performance-optimized with HTML5 Canvas rendering
// 10x12 grid, R1 Sunflower, Blue Jewel, Green Clover (board v7 designs)
// v8.12: Fixed cascade activation bug - specials triggered by combos now properly activate
// v8.13: Supernova/Hypernova popups have higher z-index and longer duration
// =============================================================================
...
const DROP_SPEED = 0.17;     // v8.8: Increased from 0.15 for faster gravity
const MATCH_FADE_SPEED = 0.15;
...
const Match3Game = () => {
  ...
  useEffect(() => {
    ...
    const checkTimer = setTimeout(() => {
      ...
      const hasReachedTarget = targetReached || currentScore >= levelTarget;
      
      console.log('Game end check:', { currentScore, levelTarget, hasReachedTarget, moves, targetReached, bonusRoundActive });
      ...
      // Target reached with no moves (no bonus prompt needed)
      ...
      // Game over: out of moves and didn't reach target
  ...
  const renderCanvas = useCallback(() => {
    ...
    grid.forEach((row, rowIndex) => {
      row.forEach((tile, colIndex) => {
        ...
        // v8.3: Check if tile should glow (before activation)
        const isGlowing = glowingTiles.some(t => t.row === rowIndex && t.col === colIndex);
        // v8.3: Check if tile should flash (during activation)
        const isFlashing = flashingTiles.some(t => t.row === rowIndex && t.col === colIndex);
        ...
        if (isGlowing) {
          ...
          ctx.shadowBlur = 15;
          ctx.strokeRect(tile.animX - 2, tile.animY - 2, TILE_SIZE + 4, TILE_SIZE + 4);
          ctx.restore();
        }
        ...
        if (isFlashing) {
          ...
          ctx.fillRect(tile.animX, tile.animY, TILE_SIZE, TILE_SIZE);
          ctx.restore();
        }
  ...
  }, [grid, selectedTile, matchedTiles, pendingSpecials, boardWidth, boardHeight, flashingTiles, glowingTiles]);
    ...
    <div style={{
      ...
      <div style={{
        ...
        <h1 style={{ margin: '0', color: '#333', fontSize: '20px', textShadow: '2px 2px 4px rgba(0,0,0,0.1)' }}>
          🎮 Match-3 <span style={{ fontSize: '12px', color: '#888' }}>v8.13</span>
        </h1>
        <div style={{
          display: 'flex',
          ...
          <div>Moves: <span style={{ color: '#667eea' }}>{moves}</span></div>
      ...
      {showBonusPrompt && (
        <div style={{
          ...
          <div style={{ 
            ...
            <div>Score: <strong>{score}</strong> / {levelTarget}</div>
          ...
          <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap', justifyContent: 'center' }}>
            ...
            >
              End (+{moves * EARLY_END_BONUS_PER_MOVE})
            </button>
          </div>
        </div>
      )}
